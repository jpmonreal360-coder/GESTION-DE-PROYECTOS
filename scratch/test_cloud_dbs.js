...
async function testServices() {
  // Test 1: npoint.io
  try {
    const res = await fetch('https://api.npoint.io/66b0a1a0acd3cb34a871', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    });
    console.log('npoint.io response:', res.status);
  } catch (e) {
    console.log('npoint.io error:', e.message);
  }

  // Test 2: jsonstorage.net
  try {
    ...
    console.log('jsonstorage response:', res.status, res.headers.get('location'));
  } catch (e) {
    console.log('jsonstorage error:', e.message);
  }
}
