...
export const KnowledgeBase: React.FC<KnowledgeBaseProps> = ({
  ...
  const handleSaveChanges = () => {
    if (currentDoc) {
      ...
      currentDoc.updatedAt = new Date().toISOString().split('T')[0];
      if (onSaveDoc) onSaveDoc(currentDoc);
      setIsSaved(true);
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-neutral-500 dark:text-neutral-400">
            Documentación técnica y notas del proyecto con guardado de cambios
          </p>
        </div>
        ...
        {currentDoc && (
          ...
          >
            ...
            <span>{isSaved ? 'Guardado Exitosamente' : 'Guardar Cambios'}</span>
          </button>
        )}
      </div>
