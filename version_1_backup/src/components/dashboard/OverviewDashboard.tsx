      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-500 mt-1">
            Resumen ejecutivo y Balance Neto (Entradas − Gastos)
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl apple-glass apple-glass-hover">
          ...
          <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
            +${totalIncome.toLocaleString()} <span className="text-xs text-neutral-400 font-normal">USD</span>
          </p>
        ...
        <div className="p-5 rounded-2xl apple-glass apple-glass-hover">
          ...
          <p className="text-2xl font-bold text-rose-600 dark:text-rose-400">
            -${totalExpenses.toLocaleString()} <span className="text-xs text-neutral-400 font-normal">USD</span>
          </p>
        ...
        <div className="lg:col-span-2 p-6 rounded-2xl apple-glass space-y-4">
          ...
          <div className="space-y-4">
            {filteredProjects.map((p) => {
              ...
              const budgetVal = p.budget ?? p.totalBudget ?? 20000;
                  ...
                  <div className="flex justify-between text-xs font-semibold">
                    ...
                    <span>
                      Balance: <strong className={prjNet >= 0 ? 'text-emerald-500' : 'text-rose-500'}>
                        {prjNet >= 0 ? '+' : ''}${prjNet.toLocaleString()} USD
                      </strong>
                    </span>
                  </div>
        ...
        <div className="p-6 rounded-2xl apple-glass space-y-4">
          ...
          <div className="space-y-3">
            {expenses.slice(0, 5).map((exp) => (
              <div key={exp.id} className="flex items-center justify-between pb-2 border-b border-neutral-200/40 dark:border-neutral-800/40">
                <div>
                  <p className="text-xs font-semibold">{exp.concept}</p>
                  <p className="text-[10px] text-neutral-400">{exp.category} • {exp.date}</p>
                </div>
                <span className={`text-xs font-bold ${exp.type === 'INCOME' ? 'text-emerald-500' : 'text-rose-500'}`}>
